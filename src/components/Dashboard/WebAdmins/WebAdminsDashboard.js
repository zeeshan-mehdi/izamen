...
import AddWebAdmin from '../Components/AddWebAdmin/AddWebAdmin'
...
function WebAdminsDashboard(props) {
  const { classes } = props;

  const [webadmincolums, setwebadmincolums] = useState([
    {
  ...
  const getallwebadmins= async ()=>{
    try {
      const res = await axios.get(globalvariables.data.Basicapi + "getAllWebAdmin",
        {
          headers: {
            'Content-Type': 'application/json',
          },
        });
      console.log("res from getting Web Admins", res.data);
      if (res.data.length > 0) {
        Setwebadmins(res.data)
    ...
    <>
      <Container fluid>
        <TableViewTemplate
          keyField={"web_ID"}
          data={webadmins}
          columns={webadmincolums}
          addButtonText={"Create Web Admin"}
          tabTitle={"Web Admins"}
          addButtonOnClick={() => {
